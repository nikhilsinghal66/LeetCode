...
public:
    TreeNode* buildtree(vector<int> &preorder,int prestart,int preend,vector<int> &inorder,int instart,int inend,map<int,int> &mpp){
        ...
        root->left=buildtree(preorder,prestart+1,prestart+numsize,inorder,instart,inpos-1,mpp);
        root->right=buildtree(preorder,prestart+numsize+1,preend,inorder,inpos+1,inend,mpp);
        return root;
